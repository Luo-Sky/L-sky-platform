...
//        urls.put("img2", resultUrl2);
    ...
    public Result<?> process(@RequestParam Long historyId, @RequestParam String flag1, @RequestParam String flag2, @RequestParam(defaultValue = "0") Integer batch, @RequestBody HistoryConfig historyConfig) throws IOException {
        ...
        String fileName1 = basePath + fileNames.stream().filter(name -> name.contains(flag1) && name.charAt(flag1.length())=='.').findAny().orElse("");;  // 找到跟参数一致的文件
        String fileName2 = basePath + fileNames.stream().filter(name -> name.contains(flag2) && name.charAt(flag2.length())=='.').findAny().orElse("");;  // 找到跟参数一致的文件
