...
import com.rs.platform.mapper.EeHistoryMapper;
...
@Service
public class EeHistoryServiceImpl extends ServiceImpl<EeHistoryMapper, EeHistory> implements IEeHistoryService {

    @Autowired
    private EeHistoryMapper eeHistoryMapper;
...
//        headers.setContentType(MediaType.APPLICATION_FORM_URLENCODED);
        ...
        try {
            ...
            EeHistory eeHistory = new EeHistory();
            eeHistory.setId(historyId);
            eeHistory.setEndTime(new Date());
            eeHistory.setResultImg(resultImg);
            eeHistory.setResultImgName("高程估计结果图");
            eeHistory.setResult(jsTemp.toJSONString());
            eeHistory.setChoose(0);
            if (eeHistoryMapper.updateById(eeHistory) == 1) {
                return jsTemp;
            } else {
                System.out.println("高程估计的保存记录操作失败");
                return null;
            }
        } catch (Exception e) {
            System.out.println(e + "高程估计发送请求异常");
        }
        return null;
    }
    ...
    public JSONObject processBoxSelection(Long historyId, String url, String fileName, String type, HistoryConfig historyConfig) {
        ...
        try {
            ...
            boxSelection.setResultImgName("高程估计结果图");
            boxSelection.setResult(jsTemp.toJSONString());

            EeHistory eeHistory = new EeHistory();
            eeHistory.setId(historyId);
            eeHistory.setChoose(1);
            if (boxSelectionMapper.insert(boxSelection) == 1 && eeHistoryMapper.updateById(eeHistory) == 1) {
                return jsTemp;
            } else {
                System.out.println("高程估计的保存记录操作失败");
                return null;
